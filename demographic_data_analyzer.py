...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(num_rich_min_workers*100/num_min_workers.shape[0], 1)

    country_salary_counts = df['native-country'].value_counts()
    country_rich_counts = df[df['salary'] == '>50K']['native-country'].value_counts()
    highest_earning_country_percentage = round((country_rich_counts / country_salary_counts * 100).max(), 1)
    highest_earning_country = (country_rich_counts / country_salary_counts).idxmax()
